...
Node *sql_transform_expr_recurse(ParseState *pstate, Node *expr);
...
sql_transform_expr(ParseState *pstate, Node *expr, ParseExprKind exprKind)
{
	...
	result = sql_transform_expr_recurse(pstate, expr);
...
Node *
sql_transform_expr_recurse(ParseState *pstate, Node *expr)
{
	Node	   *result;
	...
	switch (nodeTag(expr))
	{
		...
		case T_NamedArgExpr:
			{
				...
				na->arg = (Expr *) sql_transform_expr_recurse(pstate, (Node *) na->arg);
				result = expr;
				break;
			}
		...
		case T_NullTest:
			{
				...
				n->arg = (Expr *) sql_transform_expr_recurse(pstate, (Node *) n->arg);
		...
		case T_CurrentOfExpr:
			 ...
			 * processed it rather than passing it to sql_transform_expr().
			 */
		case T_SetToDefault:
			ereport(ERROR,
...
transformIndirection(ParseState *pstate, A_Indirection *ind)
{
	...
	Node	   *result = sql_transform_expr_recurse(pstate, ind->arg);
	List	   *subscripts = NIL;
	int			location = exprLocation(result);
	ListCell   *i;

    if (IsA(ind->arg, String)) {
        ParseNamespaceItem *pnsi;

        char *relname = strVal(ind->arg);
        int levels_up;
        // locate the referenced RTE
        pnsi = refnameNamespaceItem(pstate, NULL, relname, -1, &levels_up);
        ...
        if (pnsi) {
            ...
            Node *node = scanNSItemForColumn(pstate, pnsi, 0, "properties", -1);
            if (node)
                result = node;
        }
    }
		...
		case 2:
			{
				...
				if (node == NULL)
				{
                    ...
                    if (!node) {
                        Node *prop = scanNSItemForColumn(pstate, nsitem, levels_up, "properties", cref->location);
                        if (prop){
                            Const *c = makeConst(GTYPEOID, -1, InvalidOid, -1, string_to_gtype(colname), false, false);
                            node = (Node *)make_op(pstate, list_make2(makeString("postgraph"), makeString("->")), prop, c, pstate->p_last_srf,  -1);
                        }
                    }
				break;
			}
...
transformAExprOp(ParseState *pstate, A_Expr *a)
{
		...
		(!IsA(lexpr, CaseTestExpr) && !IsA(rexpr, CaseTestExpr)))
	{
		...
		result = sql_transform_expr_recurse(pstate, (Node *) n);
	}
			 ...
			 ((SubLink *) rexpr)->subLinkType == EXPR_SUBLINK)
	{
		...
		result = sql_transform_expr_recurse(pstate, (Node *) s);
	}
	else if (lexpr && IsA(lexpr, RowExpr) &&
			 rexpr && IsA(rexpr, RowExpr))
	{
		/* ROW() op ROW() is handled specially */
		lexpr = sql_transform_expr_recurse(pstate, lexpr);
		rexpr = sql_transform_expr_recurse(pstate, rexpr);
...
{
	Node	   *lexpr = sql_transform_expr_recurse(pstate, a->lexpr);
	Node	   *rexpr = sql_transform_expr_recurse(pstate, a->rexpr);
...
transformAExprIn(ParseState *pstate, A_Expr *a)
{
	 ...
	 * Vars.
	 */
	lexpr = sql_transform_expr_recurse(pstate, a->lexpr);
	rexprs = rvars = rnonvars = NIL;
	foreach(l, (List *) a->rexpr)
	{
		Node	   *rexpr = sql_transform_expr_recurse(pstate, lfirst(l));
...
transformAExprBetween(ParseState *pstate, A_Expr *a)
{
	...
	return sql_transform_expr_recurse(pstate, result);
}
...
transformBoolExpr(ParseState *pstate, BoolExpr *a)
{
	...
	foreach(lc, a->args)
	{
		...
		arg = sql_transform_expr_recurse(pstate, arg);
...
transformFuncCall(ParseState *pstate, FuncCall *fn)
{
	...
	{
		targs = lappend(targs, sql_transform_expr_recurse(pstate,
													(Node *) lfirst(args)));
	}
	...
	if (fn->agg_within_group)
	{
		...
		foreach(args, fn->agg_order)
		{
			...
			targs = lappend(targs, sql_transform_expr(pstate, arg->node,
												 EXPR_KIND_ORDER_BY));
		}
	}
...
transformMultiAssignRef(ParseState *pstate, MultiAssignRef *maref)
{
	...
	if (maref->colno == 1)
	{
			...
			((SubLink *) maref->source)->subLinkType == EXPR_SUBLINK)
		{
			...
			/* And transform it */
			sublink = (SubLink *) sql_transform_expr_recurse(pstate,
													   (Node *) sublink);
...
transformCaseExpr(ParseState *pstate, CaseExpr *c)
{
	...
	arg = sql_transform_expr_recurse(pstate, (Node *) c->arg);
	...
	foreach(l, c->args)
	{
		...
		neww->expr = (Expr *) sql_transform_expr_recurse(pstate, warg);
		...
		warg = (Node *) w->result;
		neww->result = (Expr *) sql_transform_expr_recurse(pstate, warg);
	...
	newc->defresult = (Expr *) sql_transform_expr_recurse(pstate, defresult);
...
transformSubLink(ParseState *pstate, SubLink *sublink)
{
	...
	else
	{
		 ...
		 */
		lefthand = sql_transform_expr_recurse(pstate, sublink->testexpr);
				   ...
				   Oid array_type, Oid element_type, int32 typmod)
{
	...
	foreach(element, a->elements)
	{
		...
		{
			newe = sql_transform_expr_recurse(pstate, e);
...
transformXmlExpr(ParseState *pstate, XmlExpr *x)
{
	...
	foreach(lc, x->named_args)
	{
		...
		expr = sql_transform_expr_recurse(pstate, r->val);
...
transformXmlSerialize(ParseState *pstate, XmlSerialize *xs)
{
	...
	xexpr->args = list_make1(coerce_to_specific_type(pstate,
													 sql_transform_expr_recurse(pstate, xs->expr),
													 XMLOID,
													 "XMLSERIALIZE"));
...
transformBooleanTest(ParseState *pstate, BooleanTest *b)
{
	...
	b->arg = (Expr *) sql_transform_expr_recurse(pstate, (Node *) b->arg);
...
transformTypeCast(ParseState *pstate, TypeCast *tc)
{
	...
	if (IsA(arg, A_ArrayExpr))
	{
		...
		if (OidIsValid(elementType))
		{
			...
			expr = sql_transform_expr_recurse(pstate, arg);
	}
	else
		expr = sql_transform_expr_recurse(pstate, arg);
...
transformCollateClause(ParseState *pstate, CollateClause *c)
{
	...
	newc->arg = (Expr *) sql_transform_expr_recurse(pstate, c->arg);
...
make_nulltest_from_distinct(ParseState *pstate, A_Expr *distincta, Node *arg)
{
	...
	nt->arg = (Expr *) sql_transform_expr_recurse(pstate, arg);
	/* the argument can be any type, so don't coerce it */
