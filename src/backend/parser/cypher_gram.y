...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	AGGREGATE ALL ALSO ALTER ALWAYS ANALYSE ANALYZE AND ANY ARRAY AS ASC ASCENDING
	...
	DATA_P DATABASE DAY_P DEALLOCATE DEC DESCENDING DECIMAL_P DECLARE DEFAULT DEFAULTS
	DEFERRABLE DEFERRED DEFINER DELETE DELIMITER DELIMITERS DEPENDS DEPTH DESC
	DETACH DICTIONARY DISABLE_P DISCARD DISTINCT DO DOCUMENT_P DOMAIN_P
	DOUBLE_P DROP

	EACH ELSE ENABLE_P ENCODING ENCRYPTED END_P ENDS ENUM_P ESCAPE EVENT EXCEPT
	...
	GENERATED GLOBAL GRANT GRANTED GRAPH GREATEST GROUP GROUPING GROUPS

	HANDLER HAVING HEADER_P HOLD HOUR_P

	IDENTITY_P IF ILIKE IMMEDIATE IMMUTABLE IMPLICIT_P IMPORT_P IN INCLUDE
	INCLUDING INCREMENT INDEX INDEXES INHERIT INHERITS INITIALLY INLINE_P
	INNER INOUT INPUT_P INSENSITIVE INSERT INSTEAD INT_P INTEGER_P
	...
	MAPPING MATCH MATERIALIZED MAXVALUE METHOD MERGE MINUTE_P MINVALUE MODE MONTH_P MOVE
	...
	OBJECT_P OF OFF OFFSET OIDS OLD ON ONLY OPERATOR_P OPTION OPTIONAL OPTIONS OR
	ORDER ORDINALITY OTHERS OUT_P OUTER
	...
	START STARTS STATEMENT STATEMENTS STATISTICS STDIN STDOUT STORAGE STORED STRICT_P STRIP_P
...
/* query */
%type <node> stmt
%type <list> cypher_query  cypher_stmt
...
%type <string>		opt_provider security_label
...
%type <string> symbolic_name schema_name attr_name 
...
%type <integer>	import_qualification_type
...
%type <string>		foreign_server_version opt_foreign_server_version
...
%type <string> Sconst comment_text notify_payload file_name
...
AlterStatsStmt:
			...
			| ALTER STATISTICS IF EXISTS any_name SET STATISTICS SignedIconst
				{
					AlterStatsStmt *n = makeNode(AlterStatsStmt);
					n->defnames = $5;
					n->missing_ok = true;
...
CreateForeignServerStmt: CREATE SERVER name opt_type opt_foreign_server_version
				...
				| CREATE SERVER IF NOT EXISTS name opt_type opt_foreign_server_version
						 FOREIGN DATA_P WRAPPER name create_generic_options
				{
					CreateForeignServerStmt *n = makeNode(CreateForeignServerStmt);
					n->servername = $6;
...
DropOpClassStmt:
			...
			| DROP OPERATOR_P CLASS IF EXISTS any_name USING name opt_drop_behavior
				{
					DropStmt *n = makeNode(DropStmt);
...
CommentStmt:
			...
			| COMMENT ON OPERATOR_P CLASS any_name USING name IS comment_text
				{
					CommentStmt *n = makeNode(CommentStmt);
					n->objtype = OBJECT_OPCLASS;
			...
			| COMMENT ON OPERATOR_P FAMILY any_name USING name IS comment_text
				{
					CommentStmt *n = makeNode(CommentStmt);
					n->objtype = OBJECT_OPFAMILY;
					n->object = (Node *) lcons(makeString($7), $5);
...
SelectStmt: select_no_parens			%prec UNARY_MINUS
...
/*
 * The overall structure of cypher_query looks like below.
 *
...
AlterTSConfigurationStmt:
			...
			| ALTER TEXT_P SEARCH CONFIGURATION any_name DROP MAPPING IF EXISTS FOR name_list
				{
					AlterTSConfigurationStmt *n = makeNode(AlterTSConfigurationStmt);
					n->kind = ALTER_TSCONFIG_DROP_MAPPING;
					n->cfgname = $5;
...
cypher_a_expr:
    ...
    | cypher_a_expr NOT LIKE cypher_a_expr
        {   
            $$ = (Node *)makeSimpleCypherA_Expr(AEXPR_OP, "!~~", $1, $4, @2);
        }  
